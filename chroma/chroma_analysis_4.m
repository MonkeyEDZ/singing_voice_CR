function [ f_CP,f_CLP,f_CENS,f_CRP,f_CRPSmoothed,f_CENSSmoothed,featureRateSmoothed ] = ...
    chroma_analysis_4(f_audio,sideinfo,midiMin,midiMax,BPM,WB,n,winSamp,...
    downSamp,featureDens,titulo)
    % busqueda de chroma en carpeta por defecto 
    % version 4 de chroma_analysis 15/01/18
    % versión específica para formantes (mód VOZ)
    % utilizacion de prametros de GUI

    % INPUT
    %   f_audio = vector de audio wav
    %   sideinfo = vector de información de wav
    %   midiMin =  límite de análisis inferior (nota midi)
    %   midiMax = límite de análisis superior (nota midi)
    ...
    % [f_audio,sideinfo] = wav_to_audio('', 'data_WAV/', filename);
     ...
     shiftFB = estimateTuning(f_audio); % movimiento de filtros de afinación
...
%    featureDens = 1.1;
     [f_pitch,sideinfo] = audio_to_pitch_via_FB_3... %pitch de formantes
